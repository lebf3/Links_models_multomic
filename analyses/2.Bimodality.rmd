...
SNX20 <- links0.01 %>% dplyr::filter(gene=="SNX20") %>% 
  arrange(desc(score))
...
p0 <- ggplot(all_bg.peaks, aes(x=bg.z, y=gene_peak, group=gene_peak)) +
  ggridges::geom_density_ridges(stat = "binline", 
                                ...
                                alpha = 0.5, 
                                # scale = 3
                                scale = 1 # changes comment Reviewer 2
                                ) +
...
ggsave(plot = p0, "../figs/2_out/density_NOD2_biomodality_all.peaks.in.null.png", width = 7, height = 6)
...
ggsave(plot = p1, "../figs/2_out/density_NOD2_biomodality_no.cor.peaks.in.null.png", width = 7, height = 6)
```
...
```{r}
Spear.cor.0.1 <- cor(links0.1$score , links0.1$zscore, method = "spearman") %>% round(3)

p <- ggplot(links0.1, aes(score, 
                          zscore, 
                          # replace 0 with min H0.p to avoid inf
                          color = -log10(H0.p + 0.001))) + 
  annotate("text", x = -0.185, y = 12,
           label = paste0("Spearman R = ", Spear.cor.0.1),
           size = 5) +
...
ggsave(plot = p, "../figs/2_out/density_biomodality_4examples.png", width = 7, height = 10)
```
